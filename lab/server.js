...
app.get("/api/health", (_request, response) => response.json({ ok: true, version: "1.4.19" }));
...
function createEditorHtml(userId = "server") {
  ...
  html = html.replace('<meta charset="utf-8" />', '<meta charset="utf-8" /><script src="/lab-viewport.js?v=1.4.19"></script>');
...
`;
  ...
  html = html.replace("</body>", `<script>window.__FILMLAB_ACCOUNT_ID__=${JSON.stringify(accountKey)}</script><script src="/lab-editor.js?v=1.4.19"></script>\n</body>`);
  return html;
}
