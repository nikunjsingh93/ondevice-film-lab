...
app.get("/api/health", (_request, response) => response.json({ ok: true, version: "1.4.13" }));
...
app.get("/lab-viewport.js", (_request, response) => response.sendFile(path.join(PUBLIC_DIR, "lab-viewport.js")));
...
function createEditorHtml(userId = "server") {
  ...
  html = html.replace('<meta charset="utf-8" />', '<meta charset="utf-8" /><script src="/lab-viewport.js?v=1.4.13"></script>');
  ...
  html = html.replace(/<title>[^<]*<\/title>/, "<title>Lab Server</title>")
    ...
    .replace(/name="application-name" content="[^"]*"/, 'name="application-name" content="Lab Server"')
    .replace(/name="theme-color" content="[^"]*"/, 'name="theme-color" content="#111111"')
    .replace(/name="apple-mobile-web-app-title" content="[^"]*"/, 'name="apple-mobile-web-app-title" content="Lab Server"');
...
`;
  ...
  html = html.replace("</body>", `<script>window.__FILMLAB_ACCOUNT_ID__=${JSON.stringify(accountKey)}</script><script src="/lab-editor.js?v=1.4.13"></script>\n</body>`);
  return html;
}
